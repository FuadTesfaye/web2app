...
export async function skillCommand(options: SkillOptions = {}) {
  if (options.print) {
    console.log(SKILL_CONTENT.trim());
    return;
  }
  ...
  const targetDir = options.out
    ? path.resolve(rootDir, options.out)
    : path.join(rootDir, ".agents", "skills", "web2app");

  const targetFile = path.join(targetDir, "SKILL.md");

  if ((await FileSystem.exists(targetFile)) && !options.force) {
    Logger.warn(`AI Skill already exists at: ${targetFile}`);
    Logger.tip("Use --force to overwrite.");
    return;
  }
